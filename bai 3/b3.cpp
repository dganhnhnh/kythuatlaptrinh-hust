...
bool check(int xx, int yy){
    if(mark[xx][yy] == 1) return false;     // đi qua rồi 
    if(xx < 1 || xx > n || yy < 1 || yy > n) return false; // đi ra ngoài tọa độ 1-n 
    //cout << xx << " " << yy << endl;
    return true;
}
...
void TRY(int k){
    for(int i = 0; i < 8; i++){
        int xx = X[k-1] + hx[i];    // di chuyển theo trục x 
        int yy = Y[k-1] + hy[i];    // di chuyển theo trục y 
